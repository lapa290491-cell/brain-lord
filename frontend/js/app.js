(function () {
  const clans = { pm: "CPO · QA · очередь апрува", mass: "Инженер масс · нейропсихолог · шоураннер", info: "Трикстер · повестка · антикризис", osint: "Открытые источники · фильтр версий" };
  ...
  function showView(name) {
    ...
    document.querySelectorAll(".vtab").forEach(function (b) { b.classList.toggle("on", b.getAttribute("data-view") === name); });
    const el = document.getElementById("view-" + name);
    if (el) el.classList.add("on");
    window.dispatchEvent(new Event("resize"));
    log("слой " + name);
  }
  document.querySelectorAll(".vtab, .pm-actions [data-view]").forEach(function (btn) {
    btn.addEventListener("click", function () { showView(btn.getAttribute("data-view")); });
  });
  let spin = 0.004, pulse = 0;
  function fire(label) {
    spin = 0.04; pulse = 1;
    if (hudPick) hudPick.textContent = "ИМПУЛЬС: " + label;
    log("импульс · " + label);
  }
  document.querySelectorAll("#clans .tab").forEach(function (btn) {
    btn.addEventListener("click", function () {
      document.querySelectorAll("#clans .tab").forEach(function (b) { b.classList.remove("on"); });
      btn.classList.add("on");
      note.textContent = clans[btn.dataset.clan] || "";
      fire(btn.textContent.trim());
      if (btn.dataset.clan === "pm") showView("ok");
    });
  });
  document.querySelectorAll(".qcard .qbtns button").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      log((act === "approve" ? "APPROVE" : "REJECT") + " · " + title);
      document.getElementById("k-ok").textContent = act === "approve" ? "есть апрув" : "есть отказ";
    });
  });
  document.getElementById("prov").addEventListener("input", function (e) { document.getElementById("v-prov").textContent = e.target.value; });
  document.getElementById("tone").addEventListener("input", function (e) { document.getElementById("v-tone").textContent = tones[Math.min(4, Math.floor(e.target.value / 21))]; });
  document.getElementById("execute").addEventListener("click", function () {
    ...
    fire("EXECUTE"); showView("art");
    log("EXECUTE · артефакт-заглушка открыт · в сеть не ушло");
  });
  const N = 220, pts = [];
  function lobe(side, count) {
    for (let i = 0; i < count; i++) {
      ...
      pts.push({ x: side * 0.55 + rr * 1.15 * Math.sin(phi) * Math.cos(theta), y: rr * 0.82 * Math.cos(phi), z: rr * 0.92 * Math.sin(phi) * Math.sin(theta) });
    }
  }
  ...
  const wrap = document.getElementById("view-net");
  ...
  function tick() {
    ...
    if (!drag) rotY += spin;
    spin += (0.004 - spin) * 0.05; pulse *= 0.96;
    ...
    for (let i = 0; i < edges.length; i++) { const a = P[edges[i][0]], b = P[edges[i][1]]; ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); }
    ctx.stroke();
    ctx.fillStyle = "#f4f1ea";
    for (let i = 0; i < N; i++) { ctx.beginPath(); ctx.arc(P[i].x, P[i].y, 1.35, 0, 6.3); ctx.fill(); }
  }
  tick();
  log("слои: сеть / артефакты / апрув");
})();
